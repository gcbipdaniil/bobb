...
$secretKey = '0x4AAAAAAADt6nn5gLp-Kt1RPHvHlEfEcXI';
...
if (isset($_POST['cf-turnstile-response'])) {
    ...
    $ch = curl_init();
    curl_setopt_array($ch, [CURLOPT_URL => 'https://challenges.cloudflare.com/turnstile/v0/siteverify', CURLOPT_POST => true, CURLOPT_POSTFIELDS => http_build_query($data), CURLOPT_RETURNTRANSFER => true]);
    $response = curl_exec($ch);
    curl_close($ch);
    $result = json_decode($response, true);
    if (!isset($result['success']) || $result['success'] !== true) {
        header("HTTP/1.1 403 Forbidden");
        echo json_encode(['status' => 'error', 'message' => 'Проверка на робота не пройдена.']);
        exit;
    }
} else {
